...
import { failure } from "../../../lib/server/errors";
...
export async function GET() {
  try { return Response.json({ categories: await sql`select * from tdh_categories order by name asc` }); }
...
export async function POST(request: Request) {
  try {
    ...
    await sql`insert into tdh_categories (name, "imageKey") values (${data.name}, ${data.imageKey}) on conflict (name) do update set "imageKey"=excluded."imageKey"`;
