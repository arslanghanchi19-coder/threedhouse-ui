...
export const productSchema = z.object({
  id:z.coerce.number().int().positive().safe(), name:z.string().trim().min(1).max(150), category:z.enum(categoryNames),
...
export const reviewSchema = z.object({
  productId:z.coerce.number().int().positive().safe(), rating:z.number().int().min(1).max(5),
...
export const deliverySchema = z.object({
  ...
  items:z.array(z.object({productId:z.coerce.number().int().positive().safe(),quantity:z.number().int().min(1).max(100),color:z.string().max(100)})).min(1).max(50),
});
